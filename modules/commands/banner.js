...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  const lengthchar = (await axios.get('https://api-1.huytran6868.repl.co/taoanhdep/data')).data
  ...
  if (handleReply.step == 1) {
    ...
    api.unsendMessage(handleReply.messageID);
    return api.sendMessage(`Bạn đã chọn tên phụ là ${event.body}, reply tin nhắn này để chọn màu ( tên tiếng anh ) ( hãy ghi no nếu bạn muốn dùng màu mặc định )`, threadID, function(err, info) {
      return global.client.handleReply.push({
        step: 5,
        name: "banner",
        author: senderID,
